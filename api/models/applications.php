...
class Applications
{
    ...
    {
        $sql = "SELECT u.id AS user_id,u.user_name as uname, u.first_name AS fname, u.last_name AS lname, j.title AS title, j.id as job_id, a.message AS message, a.status AS status FROM applications a JOIN jobs j ON a.job_id = j.id JOIN clients c ON j.client_id = c.id JOIN users u ON c.user_id = u.id WHERE a.freelancer_id = ?";
    ...
    public function get_applys_by_client_id($client_id)
    {
        ...
        j.title AS title, j.id as job_id, a.message AS message, a.status AS status, a.id AS aplly_id 
        FROM applications a 
        JOIN jobs j ON a.job_id = j.id 
        JOIN clients c ON j.client_id = c.id 
        JOIN users u ON c.user_id = u.id 
    ...
    {
        $sql = "SELECT u.id AS user_id, u.first_name AS fname, u.last_name AS lname, j.title AS title, j.description AS message, j.status AS status, j.id AS job_id FROM applications a JOIN jobs j ON a.job_id = j.id JOIN clients c ON j.client_id = c.id JOIN users u ON c.user_id = u.id WHERE a.freelancer_id = ? AND a.status = 'approve'";
