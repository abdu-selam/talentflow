    ...
    const li = `
      <li class="application__item">
          ...
          <div class="item__btns">
              ...
              ${applyBtnConstructor(item.status, item.aplly_id)}
          </div>
      </li>
  `;
...
const applyBtnConstructor = (stat, id) => {
  const btn = (stat, word) =>
    `<button data-stat="${stat}" data-id="${id}" class="item__btn">${word}</button>`;
    ...
    btn.addEventListener("click", async (e) => {
      ...
      if (res.status == 200) {
        ...
        wrapper.insertAdjacentHTML(
          "beforeend",
          applyBtnConstructor(data.status, data.id),
        );
      }
    });
  });
